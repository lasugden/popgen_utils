            ...
            for time in params['sweep_time']:
                ...
                if len(Svec)>0:
                    if not opath.exists(opath.join(output_path, parameter_model_name+'_viterbi_classified.txt')):
                        HMM.viterbi(Svec, positions, output_path, parameter_model_name, plotpaths=False, classify=True)
                    if not opath.exists(opath.join(output_path, parameter_model_name+'_hmm_classified.txt')):
                        HMM.many_backtraces(Svec, 1000, positions, output_path, parameter_model_name, plotpaths=False, plotdensity=False, classify=True, plot_classify=False)
